...
@section('title', 'Open Matches')

@section('content')
    <div class="w-content mx-auto md:py-8 py-4 flex flex-col md:gap-6 gap-4">
        <div class="flex gap-4 ">
            <form action="{{ route('event.open-matches.index') }}" class="flex items-center gap-4 grow w-full justify-between flex-wrap">
                <div class="flex items-center gap-2 flex-wrap grow md:max-w-sm flex-1 min-w-xs">
                    ...
                    @if (request('search'))
                        <a href="{{ route('event.open-matches.index') }}" title="Reset Pencarian"
                            class="bg-primary text-white h-12 aspect-square cursor-pointer hover:brightness-90 rounded-full flex items-center justify-center">
        ...
        <div class="grid lg:grid-cols-3 md:grid-cols-2 md:gap-8 gap-4">
            @forelse ($openMatches as $openMatch)
                <div class="bg-white p-6 rounded-lg shadow-xl flex flex-col gap-3">
                    <div class="aspect-video">
                        <img src="{{ $openMatch->booking->field->getImageUrlAttribute() }}" alt="open match"
                            class="aspect-video w-full rounded-md object-cover bg-zinc-200">
                    </div>
                    <h3 class="font-semibold text-[20px]">{{ $openMatch->match_name }}</h3>
                    ...
                    <div class="flex items-center gap-2">
                        ...
                        <span class="text-sm">
                            {{ $openMatch->booking->field->fullLocation }}
                        </span>
                    </div>
                    ...
                    <a href="{{ route('event.open-matches.show', $openMatch->id) }}" class="btn btn-primary">
                        Lihat Open Match
                    </a>
                </div>
            @empty
                ...
                @else
                    <div class="col-span-3 text-center bg-white p-8 rounded-lg shadow-xl flex flex-col items-center gap-4">
                        ...
                        <h2 class="text-xl font-semibold">Open match tidak tersedia</h2>
                        <p class="text-zinc-500 text-sm">
                            Saat ini tidak ada open match yang tersedia. Silakan cek kembali nanti atau <a href="{{ route('home') }}" class="text-primary hover:underline">buat open match baru</a>.
                        </p>
                    </div>
                @endif
            @endforelse
